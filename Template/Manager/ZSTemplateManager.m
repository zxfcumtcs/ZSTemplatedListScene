...
#import "ZSTemplateManager.h"
#import "ZSTemplateAPI.h"
#import "ZSModule.h"

static NSString *ZSTemplateCacheFileName = @"cache";

@interface ZSTemplateManager ()
...
@implementation ZSTemplateManager
...
- (void)_setupCacheDir
{
    ...
    _cacheDir = [supportPath stringByAppendingPathComponent:@"Template"];
...
- (void)loadCacheData:(void (^)(NSError *error, id result))callback
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSData *cacheData = [NSData dataWithContentsOfFile:[self->_cacheDir stringByAppendingPathComponent:ZSTemplateCacheFileName]];
...
- (void)_storageData:(id)data
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSString *path = [self->_cacheDir stringByAppendingPathComponent:ZSTemplateCacheFileName];
...
#warning set params if need
    ...
    [ZSTemplateAPI requestTemplateWithParams:nil
